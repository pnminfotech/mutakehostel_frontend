...
function Add_Data_Demo() {
 ...
 const [selectedYear, setSelectedYear] = useState(new Date().getFullYear()); // Default: current year
  ...
  const apiUrl = '  https://mutakehostel-api.pnminfotech.com/api/'; // Replace with your actual API endpoint
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      ...
      const fetchData = async () => {
        try {
          const response = await fetch(`  https://mutakehostel-api.pnminfotech.com/api/form/${id}`);
    ...
    const handleDownloadForm = async (data) => {
      try {
        const response = await axios.get(`  https://mutakehostel-api.pnminfotech.com/api/form/${data._id}`);
  ...
  try {
    ...
    const response = await fetch(
      `  https://mutakehostel-api.pnminfotech.com/api/light-bill/all-bills?month=${currentMonth}&year=${currentYear}`
    );
    const data = await response.json();
  ...
  useEffect(() => {
    ...
    idsToArchive.forEach((id) => {
      ...
      if (recordToArchive) {
        // Archive the record
        axios
          .post(`  https://mutakehostel-api.pnminfotech.com/api/forms/archive`, { id })
          .then((response) => {
  ...
  useEffect(() => {
    axios
      .get('  https://mutakehostel-api.pnminfotech.com/api/forms/archived')
      .then((response) => {
  ...
  useEffect(() => {
    axios
      .get("  https://mutakehostel-api.pnminfotech.com/api/forms")
      .then((response) => {
        const leaveData = {};
...
//   };
  ...
  const confirmLeave = async () => {
    ...
    try {
      const response = await fetch("  https://mutakehostel-api.pnminfotech.com/api/leave", {
        method: "POST",
  ...
  const handleUpdate = () => { 
    ...
    if (newRentAmount === 0) {
      // 📌 If Rent Amount is 0, Call DELETE API   https://mutakehostel-api.pnminfotech.com
      axios
        .delete(`  https://mutakehostel-api.pnminfotech.com/api//api/form/${currentFormId}/rent/${monthYear}`)
        .then(() => {
  ...
  axios
  .put(`  https://mutakehostel-api.pnminfotech.com/api/form/${currentFormId}`, updatedRent)
  .then((response) => {
    const patched = {
      ...response.data,
  ...
  const handleDeleteConfirm = async () => {
    try {
      await axios.delete(`  https://mutakehostel-api.pnminfotech.com/api/form/${currentDeleteId}`);
...
const undoArchive = (data) => {
  axios
    .post(`  https://mutakehostel-api.pnminfotech.com/api/forms/restore`, { id: data._id })
    .then(() => {
  ...
  const handleFormUpdate = () => {
    ...
    axios
      .put(`  https://mutakehostel-api.pnminfotech.com/api/update/${currentFormId}`, updatedFormData)
      .then((response) => {
