...
const MaintenanceManager = () => {
 ...
 const handleSubmit = async () => {
  ...
  const handleUpdateSubmit = async () => {
    try {
      const res = await fetch(`https://hostelpaymentmanger.onrender.com/api/light-bill/${selectedBill._id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...selectedBill,
...
const handleDelete = async (bill) => {
  ...
  if (window.confirm("Are you sure you want to delete this light bill?")) {
    try {
      ...
      const response = await fetch(`https://hostelpaymentmanger.onrender.com/api/light-bill/${bill._id}`, {
        method: 'DELETE',
        headers: {
