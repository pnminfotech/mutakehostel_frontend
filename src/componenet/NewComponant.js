...
function NewComponant() {
  ...
  const shareData = { title: 'Tenant Form', text: 'Please fill out this tenant form:', url };
...
const monthStr = new Date(currentYear, currentMonth, 1).toLocaleString("en-US", opts);
                            ...
                            <button
                              ...
                              onClick={async () => {
                                ...
                                try {
                                  ...
                                  // );
await axios.post(
  `${apiUrl}rooms/${encodeURIComponent(roomNo)}/bed`,
  payload
);
...
/>
  ...
  onChange={(e) =>
    setNewTenant((prev) => ({ ...prev, rentAmount: e.target.value }))
  }
/>
                    </div>
  ...
  onClick={async () => {
    try {
      ...
      const payload = {
        ...
        bedNo: newTenant.bedNo && newTenant.bedNo !== "__other__" ? newTenant.bedNo : undefined,
        baseRent: newTenant.baseRent ?? undefined,
        rentAmount: newTenant.rentAmount ?? newTenant.baseRent ?? undefined,
      ...
      if (!url) { alert("Failed to create invite link"); return; }
