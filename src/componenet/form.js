...
const isTenant = params.get("tenant") === "true";
  ...
  const fetchSrNo = async () => {
    try {
      const response = await axios.get('   https://mutakegirlshostel-0ko7.onrender.com/api/forms/count');
...
const handleSubmit = async (e) => {
  e.preventDefault();
  try {
    await axios.post('   https://mutakegirlshostel-0ko7.onrender.com/api/forms', formData);

    // Tenant via shared link
    if (isTenant) {
      navigate('/form-submitted');  // ✅ Redirect tenant to FormSubmitted page
    } else {
      // Admin/staff normal flow
      navigate('/add-data');
    }
...
{/* Show Back + Share only if NOT tenant */}
{!isTenant && (
  <>
    <button
      type="button"
    ...
    <button
      ...
      onClick={() => {
        const shareData = {
          ...
          url: window.location.origin + '/HostelManager/form?tenant=true',
